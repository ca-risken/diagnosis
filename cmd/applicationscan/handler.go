...
func (s *sqsHandler) HandleMessage(ctx context.Context, sqsMsg *sqs.Message) error {
	...
	if err := s.putFindings(ctx, scanResult, msg, setting.Target); err != nil {
		appLogger.Errorf(ctx, "Faild to put findings. ApplicationScanID: %v, error: %v", msg.ApplicationScanID, err)
		return s.handleErrorWithUpdateStatus(ctx, msg, err)
	}

	// Put ApplicationScan
	if err := s.putApplicationScan(ctx, msg.ApplicationScanID, msg.ProjectID, true, ""); err != nil {
		appLogger.Errorf(ctx, "Faild to put applicationscan. ApplicationScanID: %v, error: %v", msg.ApplicationScanID, err)
